...
//----------加盟模块 管理员后台接口----------
// 获取全部加盟申请列表
app.get('/api/admin/join/apply/list', auth.authAdmin, (req,res)=>{
  const list = db.prepare(`SELECT * FROM join_apply ORDER BY create_at DESC`).all();
  list.forEach(item=>{
...
// 审核加盟申请（通过/驳回，填写备注）
app.post('/api/admin/join/apply/op', auth.authAdmin, (req,res)=>{
  const {id,status,admin_note,inner_note} = req.body;
  db.prepare(`UPDATE join_apply SET status=?,admin_note=?,inner_note=?,update_at=CURRENT_TIMESTAMP WHERE id=?`)
    .run(status,admin_note||'',inner_note||'',id)
  return res.json({ok:true,msg:'操作完成'})
})

// 新增/编辑正式线下教学点档案
app.post('/api/admin/offline_shop/save', auth.authAdmin,(req,res)=>{
  const {id,org_name,province,city,district,brief,cover_img,show_status} = req.body;
  if(id){
    db.prepare(`UPDATE offline_shop SET org_name=?,province=?,city=?,district=?,brief=?,cover_img=?,show_status=? WHERE id=?`)
      .run(org_name,province,city,district,brief,cover_img,show_status,id)
  }else{
    db.prepare(`INSERT INTO offline_shop(org_name,province,city,district,brief,cover_img,show_status) VALUES (?,?,?,?,?,?,?)`)
      .run(org_name,province,city,district,brief,cover_img,show_status??1)
  }
...
//==================== 加盟模块 用户端接口 直接粘贴到index.js ====================
/**
 * @POST /api/join/apply 提交加盟申请
 */
app.post('/api/join/apply', auth.authUser, (req, res) => {
  const {
    real_name, phone, province, city, district,
    has_site, site_area, site_capacity, teach_bg,
...
/**
 * @GET /api/join/my 我的加盟申请记录
 */
app.get('/api/join/my', auth.authUser, (req,res)=>{
  const userId = req.user.id;
  const list = db.prepare(`SELECT id,status,admin_note,create_at FROM join_apply WHERE user_id=? ORDER BY create_at DESC`).all(userId);
  return res.json({ok:true,data:list});
});

/**
 * @GET /api/join/detail/:applyId 查看单条申请详情（用户端）
 */
app.get('/api/join/detail/:applyId', auth.authUser, (req,res)=>{
  const applyId = Number(req.params.applyId);
  const row = db.prepare(`SELECT * FROM join_apply WHERE id=? AND user_id=?`).get(applyId,req.user.id);
  if(!row) return res.status(404).json({ok:false,msg:'记录不存在'});
  row.img_list = JSON.parse(row.img_list||'[]');
  delete row.inner_note;
  return res.json({ok:true,data:row});
});

/**
 * @GET /api/join/shop/list 获取对外展示的线下教学点列表
 */
app.get('/api/join/shop/list', (req,res)=>{
  const list = db.prepare(`SELECT id,org_name,province,city,district,brief,cover_img FROM offline_shop WHERE show_status=1`).all();
  return res.json({ok:true,data:list});
});

/**
 * @POST /api/join/shop/book 预约线下体验
 */
app.post('/api/join/shop/book',auth.authUser,(req,res)=>{
  const {shop_id,user_name,phone,demand} = req.body;
  if(!shop_id||!user_name||!phone){
    return res.status(400).json({ok:false,msg:'信息不全'});
  }
  db.prepare(`INSERT INTO offline_book(user_id,shop_id,user_name,phone,demand) VALUES (?,?,?,?,?)`)
    .run(req.user.id,shop_id,user_name,phone,demand||'');
  return res.json({ok:true,msg:'预约提交成功，机构会联系您'});
});
//==================== 用户端接口结束 ====================
